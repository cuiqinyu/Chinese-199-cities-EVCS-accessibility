...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Compute Gaussian 2SFCA accessibility for the bundled single-city "
            "sample at 1, 1.5, and 2-8 km thresholds."
        )
    )
    parser.add_argument(
        "--max-units",
...
def main() -> None:
    ...
    print("Computing Gaussian 2SFCA accessibility...")
    sfca = compute_2sfca(G, pop, evcs, snap)
    sfca_path = OUTPUT_DIR / "single_city_2sfca_1to8km_distance_threshold.csv"
    write_2sfca_public(sfca, sfca_path)

    report = build_base_report(pop, evcs, G)
    report["task"] = "Gaussian 2SFCA accessibility"
    report["outputs"]["two_sfca"] = {
        "path": format_report_path(sfca_path),
        "sha256": sha256(sfca_path),
        "rows": int(len(sfca)),
    }
    report["runtime_seconds"] = round(time.time() - start, 3)

    report_path = OUTPUT_DIR / "two_sfca_run_report.json"
    report_path.write_text(json.dumps(report, ensure_ascii=False, indent=2), encoding="utf-8")

    print("Run finished.")
    print(f"Output: {sfca_path}")
